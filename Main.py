...
def cross_section(theta_guess,m_guess,data,cross_guess):
    ...
    def p_cross(E,theta, m, L, cross):
        return 1 - cross*(sp.sin(2*theta)**2)*sp.sin((1.267*(m**2)*L)/E)**2
    ...
    def para_theta_cross(y,m,function,cross):
        x = sp.copy(y)
        #print("x is", x, "m is", m)
        x_3_old = x[1]
        y = function(x,m,data,cross)
    ...
    #print("y is" , y)
        ...
        while abs(x_3_new - x_3_old) > 1e-5:
            ...
            y = list(function(x,m,data,cross))
    ...
    def para_m_cross(y,theta,function,cross):
        ...
        while abs(x_3_new - x_3_old) > 1e-5:
            ...
            y = list(function(theta,x,data,cross))
            x_3_new = x_3(x,y)
            x = x_replace(x_3_new,x,y)
        x.sort()
    ...
    def para_cross_cross(y,m,function,cross):
       
        x = sp.copy(cross)
        #print("x is", x, "m is", m)
        x_3_old = x[1]
        y = function(theta,m,data,x)
    ...
    def univariate_cross(theta_guess,m_guess,data,function,cross_guess):
        ...
        while difference(function,data, theta_old, theta_new, m_old, m_new,cross_old,cross_new) > 0.000000001:
            looper += 1
            print("this is loop number", looper)
...
cross_section(sp.array([0,0.5,0.7]),sp.array([0.001,0.002,0.004]),data,sp.array([0,0.5,1]))